...
import sys
import os
ogbench_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'ogbench')
...
class OGBenchWrapper:
    ...
    def __init__(
        ...
        self.env = ogbench.make_env_and_datasets(
            dataset_name,
            env_only=True,
            **env_kwargs
        )
    ...
    def reset(self, env_ids: Optional[List[int]] = None) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        if not self.headless and self._is_cube_env and not self._viewer_launched:
            if hasattr(self._unwrapped_env, 'launch_passive_viewer'):
                ...
                import mujoco
                if hasattr(self._unwrapped_env, '_passive_viewer_handle') and self._unwrapped_env._passive_viewer_handle:
                    mujoco.mjv_defaultFreeCamera(self._unwrapped_env._model, self._unwrapped_env._passive_viewer_handle.cam)
