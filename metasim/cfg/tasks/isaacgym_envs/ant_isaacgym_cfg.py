...
class AntIsaacGymCfg(BaseTaskCfg, IsaacGymTaskBase):
    ...
    def get_observation(self, states):
        ...
        for i, env_state in enumerate(states):
            ...
            torso_pos = torch.tensor(robot_state.get("pos", [0., 0., self.initial_height]), dtype=torch.float32, device=device)
            torso_rot = torch.tensor(robot_state.get("rot", [1., 0., 0., 0.]), dtype=torch.float32, device=device)
            lin_vel = torch.tensor(robot_state.get("lin_vel", robot_state.get("vel", [0., 0., 0.])), dtype=torch.float32, device=device)
            ang_vel = torch.tensor(robot_state.get("ang_vel", [0., 0., 0.]), dtype=torch.float32, device=device)
            ...
            if self._targets is not None and i < len(self._targets):
                ...
                target = torch.tensor([1000., 0., 0.], dtype=torch.float32, device=torso_pos.device)
            ...
            if "sensor_forces" in robot_state:
                sensor_forces = torch.tensor(robot_state["sensor_forces"], dtype=torch.float32, device=device).flatten()[:24]
    ...
    def reward_fn(self, states, actions):
        if hasattr(states, '__class__') and states.__class__.__name__ == 'TensorState':
            ...
            if self._targets is None:
                ...
                self._targets = torch.tensor([1000., 0., 0.], device=torso_pos.device).repeat((num_envs, 1))
            ...
            heading_reward = torch.where(heading_proj > 0.8, heading_weight_tensor, self.heading_weight * heading_proj / 0.8)
            ...
            total_reward = progress_reward + alive_reward + up_reward + heading_reward - \
                self.actions_cost_scale * actions_cost - self.energy_cost_scale * electricity_cost - \
                dof_at_limit_cost * self.joints_at_limit_cost_scale
            
            total_reward = torch.where(torso_pos[:, 2] < self.termination_height, 
                                      torch.ones_like(total_reward) * self.death_cost, total_reward)
    ...
    def reset(self, env_ids=None):
        ...
        if self._targets is None:
            self._targets = torch.tensor([1000., 0., 0.], dtype=torch.float32).repeat((num_envs, 1))
    ...
    def initialize_buffers(self, num_envs, device):
        self._targets = torch.tensor([1000., 0., 0.], device=device).repeat((num_envs, 1))
        dt = 0.01667
        self._potentials = torch.zeros(num_envs, device=device) - 1000. / dt
        self._prev_potentials = self._potentials.clone()
        
        start_rot = torch.tensor([1.0, 0.0, 0.0, 0.0], device=device)
        self._inv_start_rot = quat_inv(start_rot).repeat((num_envs, 1))
        
        self._basis_vec0 = torch.tensor([1., 0., 0.], device=device).repeat((num_envs, 1))
        self._basis_vec1 = torch.tensor([0., 0., 1.], device=device).repeat((num_envs, 1))
