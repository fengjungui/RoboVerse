...
class AnymalTerrainCfg(BaseTaskCfg):
    ...
    default_joint_angles = {
        "LF_HAA": 0.03, "LH_HAA": 0.03, "RF_HAA": -0.03, "RH_HAA": -0.03,
        "LF_HFE": 0.4, "LH_HFE": -0.4, "RF_HFE": 0.4, "RH_HFE": -0.4,
    ...
    def get_observation(self, states):
        ...
        for i, env_state in enumerate(states):
            ...
            base_quat = torch.tensor(robot_state.get("rot", [1., 0., 0., 0.]), dtype=torch.float32, device=device)
            base_lin_vel = torch.tensor(robot_state.get("lin_vel", robot_state.get("vel", [0., 0., 0.])), dtype=torch.float32, device=device)
            base_ang_vel = torch.tensor(robot_state.get("ang_vel", [0., 0., 0.]), dtype=torch.float32, device=device)
            base_pos = torch.tensor(robot_state.get("pos", [0., 0., 0.62]), dtype=torch.float32, device=device)
            ...
            gravity_vec = torch.tensor([0., 0., -1.], dtype=torch.float32, device=device)
            ...
            commands_scaled = commands * torch.tensor([self.lin_vel_scale, self.lin_vel_scale, self.ang_vel_scale], device=device)
            ...
            heights_scaled = torch.clip(base_pos[2] - 0.5 - heights, -1, 1.) * self.height_meas_scale
    ...
    def reward_fn(self, states, actions):
        if hasattr(states, '__class__') and states.__class__.__name__ == 'TensorState':
            ...
            gravity_vec = torch.tensor([0., 0., -1.], device=base_pos.device).repeat(base_pos.shape[0], 1)
            projected_gravity = quat_rotate_inverse(base_quat, gravity_vec)
            ...
            if hasattr(robot_state, 'contact_forces'):
                ...
                feet_contact = contact_forces[:, -4:, 2] > 1.
                if self._feet_air_time is not None:
                    first_contact = (self._feet_air_time > 0.) * feet_contact
                    self._feet_air_time += 0.02
                    rew_air_time = torch.sum((self._feet_air_time - 0.5) * first_contact, dim=1) * self.feet_air_time_reward_scale
                ...
                stumble = (torch.norm(contact_forces[:, -4:, :2], dim=2) > 5.) * (torch.abs(contact_forces[:, -4:, 2]) < 1.)
            ...
            if self._last_actions is not None:
                rew_action_rate = torch.sum(torch.square(self._last_actions - self._actions), dim=1) * self.action_rate_reward_scale
            else:
            ...
            total_reward = rew_lin_vel_xy + rew_ang_vel_z + rew_lin_vel_z + rew_ang_vel_xy + \
                          rew_orient + rew_base_height + rew_torque + rew_joint_acc + \
                          rew_collision + rew_action_rate + rew_air_time + rew_hip + rew_stumble
    ...
    def termination_fn(self, states):
        if hasattr(states, '__class__') and states.__class__.__name__ == 'TensorState':
            ...
            terminations = torch.zeros(robot_state.root_state.shape[0], dtype=torch.bool, device=robot_state.root_state.device)
            
            if hasattr(robot_state, 'contact_forces'):
                contact_forces = robot_state.contact_forces
                base_contact = torch.norm(contact_forces[:, 0, :], dim=1) > 1.
    ...
    def reset(self, env_ids=None):
        ...
        device = self._commands.device if self._commands is not None else torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        
        commands = torch.zeros((num_envs, 4), dtype=torch.float32, device=device)
        commands[:, 0] = torch.rand(num_envs, device=device) * (self.command_x_range[1] - self.command_x_range[0]) + self.command_x_range[0]
        commands[:, 1] = torch.rand(num_envs, device=device) * (self.command_y_range[1] - self.command_y_range[0]) + self.command_y_range[0]
        commands[:, 2] = torch.rand(num_envs, device=device) * (self.command_yaw_range[1] - self.command_yaw_range[0]) + self.command_yaw_range[0]
        commands[:, 3] = 0.0
    ...
    def initialize_buffers(self, num_envs, device):
        ...
        self._episode_sums = {}
        for key in ["lin_vel_xy", "lin_vel_z", "ang_vel_z", "ang_vel_xy", "orient", "torques", 
                   "joint_acc", "base_height", "air_time", "collision", "stumble", "action_rate", "hip"]:
