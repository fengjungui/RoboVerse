...
class DMControlEnv:
    ...
    def reset(self, states: Optional[List[Any]] = None, env_ids: Optional[List[int]] = None) -> Tuple[Dict[str, torch.Tensor], None]:
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor, None]:
    ...
    def observation_space(self):
        ...
        return gym.spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(int(self.task.observation_space["shape"][0]),),
            dtype=np.float32
        )
